...
class CAJParser(object):
    ...
    def _convert_hn(self, dest):
        ...
        for i in range(self.page_num):
            rec_pos = self._TOC_END_OFFSET + i * 20
            ...
            if (page_data_offset < 0 or size_of_text_section < 0 or
                    page_data_offset + max(size_of_text_section, 0) > file_size or
                    images_per_page < 0 or images_per_page > 32):
                print("Page %d looks invalid, skipping" % (i + 1))
                continue
            ...
            for j in range(images_per_page):
                ...
                if (offset_to_image_data != current_offset + 12 or
                        size_of_image_data <= 0 or
                        offset_to_image_data + size_of_image_data > file_size):
                    print("unusual image offset on page %d, skip remaining images" % (i + 1))
                    break
